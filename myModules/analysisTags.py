...
def plotBar(data, labels, title, filepath):
    total = data[-1]
    data = data[:-1]
    labels = labels[:-1]
...
def plotFrequencyBar(classDict, filepath, pos='noun', num=30, wo=False, period=1):
    ...
    title = f"[Period {period}] Frequency in '{pos}' (most {num}) [{m}]"
    plt.title(title, fontsize=15)
    plt.savefig(f'{filepath + title}.png')
    plt.show()